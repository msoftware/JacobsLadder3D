...
public class StrategiesImpl {
	...
	public static void addButtonsToStrategies(GraphFrame graphFrame) {
		...
		addButtonRow(graphFrame, 1, GoalStatsAlgorithm.FrustrationPerCapacity, goalStatsMapping, stat -> 0.02f*((float)stat.cumulativeFrustration)/((float)(stat.capacity)));
		...
		addButtonRow(graphFrame, 2, IdentityGridStatsAlgorithm.FoundAdjacentTarget, identityGridStatsMapping, stat -> (float)(stat.foundAdjacentTarget)/2000f);
		...
		addButtonRow(graphFrame, 3, DriverStatsAlgorithm.PriorityCubeUnableMove, driverStatsMapping, stat -> ((float)stat.priorityCubeCouldNotMove)/10000f);
