...
#ifdef DEBUG_MODE_TIMER
	END_TIMER(Timer::BUILD_COLORS_INIT);
	Timer::startTimer();
#endif
		...
		for(GLuint y = 0; y < Y_BLOCKS_PLUS_1; y++)
		{
			...
			for(GLuint z = 0; z < Z_BLOCKS_PLUS_1; z++)
			{
				...
				for(GLubyte face = 0; face < NUM_FACES; face++)
				{
					...
					mergeColors(distancePaints[x][y][z][face], baseFaceColors, backgroundColor, fractionZ);
				}
			}
		}
	}

	LOG_INFO_DEBUG("  Finished creating color bands in distancePaints (%d x, %d y, %d z)", X_BLOCKS_PLUS_1, Y_BLOCKS_PLUS_1, Z_BLOCKS_PLUS_1);


#ifdef DEBUG_MODE_TIMER
	END_TIMER(Timer::BUILD_COLORS_DERIVE);
#endif
...
void ColorSet::createDistancePaintsArray()
{
	...
	distancePaints = new color_array***[X_BLOCKS_PLUS_1];

	for(GLuint x = 0; x < Y_BLOCKS_PLUS_1; x++){
		distancePaints[x] = new color_array**[Y_BLOCKS_PLUS_1];

		for(GLuint y = 0; y < Y_BLOCKS_PLUS_1; y++){
			distancePaints[x][y] = new color_array*[Z_BLOCKS_PLUS_1];

			for(GLuint z = 0; z < Z_BLOCKS_PLUS_1; z++){
				distancePaints[x][y][z] = new color_array[NUM_FACES];
			}
		}
	}
...
void ColorSet::deleteAllArrays()
{
	...
	if(distancePaints){
		...
		LOG_INFO_DEBUG("   Deleting Distance Paints... %p", distancePaints);
		...
		for(GLuint x = 0; x < X_BLOCKS_PLUS_1; x++){
			...
			for(GLuint y = 0; y < Y_BLOCKS_PLUS_1; y++){
				...
				for(GLuint z = 0; z < Z_BLOCKS_PLUS_1; z++){
					...
					delete [] distancePaints[x][y][z];
				}
				delete [] distancePaints[x][y];
			}
			delete [] distancePaints[x];
		}

		delete [] distancePaints;
