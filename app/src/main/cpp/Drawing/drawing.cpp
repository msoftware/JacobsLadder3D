...
Drawing::Drawing()
{
	LOG_INFO_DEBUG("    Creating Engine...");
...
Drawing::~Drawing()
{
	LOG_INFO_DEBUG("    Deleting Engine arrays..");

	LOG_INFO_DEBUG("    Finished deleting Engine arrays.");
		...
		Goals* goalSet)
{
	...
	if(ColorSet::getCurrentColorSet()->currentSetHasBackground()){
	...
	for(unsigned short z_ordinal = 0; z_ordinal < z_blocks; z_ordinal++){
		...
		if(_identityGrid->_sliceIsDirty[zCorrect]){
			...
			for(unsigned short xNormal = 0; xNormal < x_blocks; xNormal++){
				...
				for(unsigned short yNormal = 0; yNormal < y_blocks; yNormal++){
					...
					thisIdentity = _identityGrid->getGridElement(xNormal, yNormal, zCorrect);
					assert(thisIdentity);
		...
		_identityGrid->_sliceIsDirty[zCorrect] = false;
...
//=======================================================================
//draw moving cubes------------------------------------------------------
void Drawing::drawMovingCube(GLfloat angleOfRotation, CubePos* c){
	GLfloat xPosf = 0.0f,
			yPosf = 0.0f,
			zPosf = 0.0f;
...
void Drawing::renderGoalBoxes(GLfloat angleOfRotation, const Goals *goalSet) {
