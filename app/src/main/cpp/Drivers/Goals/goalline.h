	...
	GoalLine(GLuint u, GLuint v,
		...
		switch(_axis){
			case X_DIR:
				...
				st = (v - dist_v);
				y_start = static_cast<GLuint>(st < 0 ? 0 : st);
				y_end = v + dist_v;
				if(y_end > max_y) y_end = max_y;
				y = v;

				st = (u - dist_u);
				z_start = static_cast<GLuint>(st < 0 ? 0 : st);
				z_end = u + dist_u;
			...
			case Y_DIR:
				...
				st = (u - dist_u);
				x_start = static_cast<GLuint>(st < 0 ? 0 : st);
				x_end = u + dist_u;
		...
		availableTargets = new IterativeCollection<Identity>(expectedTargetCountInCube, TARGET_LIST);
		filledTargets = new IterativeCollection<Identity>(expectedTargetCountInCube, TARGET_LIST);
	}
