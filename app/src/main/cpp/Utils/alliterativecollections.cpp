...
void AllIterativeCollections::initializeArrays(){
	...
	if(!selectGroup)
		selectGroup = new IterativeCollection<CubePos>(LARGE_ARRAY_SIZE, SELECTED_TO_MOVE_LIST);

	if(!priorityGroup)
		priorityGroup = new IterativeCollection<CubePos>(LARGE_ARRAY_SIZE, PRIORITY_LIST);

	if(!movingElements)
		movingElements = new IterativeCollection<CubePos>(DEFAULT_ARRAY_SIZE, MOVING_LIST);

	if(!candidateGroup)
		candidateGroup = new IterativeCollection<CubePos>(LARGE_ARRAY_SIZE, CANDIDATE_LIST);
...
bool AllIterativeCollections::addCubeToList(CubePos *cube, const AddedToListType type)
{
...
GLuint AllIterativeCollections::moveUnsupportedToPriorityList(IterativeCollection<CubePos> *listToCheck){
