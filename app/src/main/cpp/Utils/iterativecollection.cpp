...
template<class T>
IterativeCollection<T>::IterativeCollection(GLuint size, AddedToListType type)
{
...
T *IterativeCollection<T>::getRandomEntry()
{
	...
	T *selectedCube = placeholder->getItem();
...
template<class T>
T *IterativeCollection<T>::getRandomEntry(GLuint upToIndex){
	GLuint restrictedPointer = _pointer < upToIndex ? _pointer : upToIndex;
	if(upToIndex == 0)
		return NULL;

	GLint selectionIndex = (GLint)((float)(upToIndex)*(float)rand()/(float)RAND_MAX);
...
template class IterativeCollection<CubePos>;
template class IterativeCollection<Identity>;
